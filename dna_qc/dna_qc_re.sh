...
# Run fastqc
for Strain in archive_S3; do
    RawData=$(ls ../../projects/hop/archive_raw_data/$Strain/R/*.fq.gz)
    echo $RawData;
    ProgDir=/home/hajduk/gitrepos/scripts/hop_analysis/dna_qc
    sbatch $ProgDir/fastqc.sh $RawData 
done

for Strain in archive_S3; do
    RawData=$(ls ../../projects/hop/archive_raw_data/$Strain/F/*.fq.gz)
    echo $RawData;
    ProgDir=/home/hajduk/gitrepos/scripts/hop_analysis/dna_qc
    sbatch $ProgDir/fastqc.sh $RawData 
done

for Strain in archive_S1; do
    RawData=$(ls ../../projects/hop/archive_raw_data/$Strain/R/*.fq.gz)
    echo $RawData;
    ProgDir=/home/hajduk/gitrepos/scripts/hop_analysis/dna_qc
    sbatch $ProgDir/fastqc.sh $RawData 
